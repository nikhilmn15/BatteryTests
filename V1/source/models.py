...
def model_comp(X_train, X_test, y_train, y_test, candidates_dict, task_name="Task"):
    """
    Trains multiple candidate estimators for a target task, prints an evaluation 
    matrix, and automatically extracts the optimal model.
    """
    ...
    print(f"\nComapring models for [{task_name.upper()}]")
    print("-" * 75)
    ...
    print(f"🏆 CHAMPION FOR {task_name.upper()}: {best_name} (Optimized via {metric_to_optimize})")
    print("-" * 75)
    return best_instance, best_name, best_metrics
...
def run_battery_ml_pipeline():
    ...
    MODEL_SAVE_DIR = Path(r"C:\Users\Nikhil\OneDrive\Desktop\BatteryTests\V1\models")
    MAIN_FILE = PROCESSED_DIR / "ProcessedV1.csv"
    ...
    soh_candidates = {
        "Linear L2 Regressor": Ridge(alpha=1.0),
    ...
    best_rul_model, rul_name, rul_metrics = model_comp(
    ...
    replace_candidates = {
        ...
        "KMeans": KNeighborsClassifier(n_neighbors=5, n_jobs=-1),
        "Random Forest": RandomForestClassifier(n_estimators=30, class_weight='balanced', random_state=42, n_jobs=-1),
        "Gradient Boosting": GradientBoostingClassifier(n_estimators=30, random_state=42)
    }
    best_replace_model, replace_name, replace_metrics = model_comp(
    ...
    artifacts_to_save = {
        "scaler.pkl": scaler,
        "kmeans": kmeans,
        "anomalydetection.pkl": iso_forest,
        "soh.pkl": best_soh_model,
        "rul.pkl": best_rul_model,
        "replacement.pkl": best_replace_model
    }
